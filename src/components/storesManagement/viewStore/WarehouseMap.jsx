...
const WarehouseMap = ({ onWarehouseSelect, selectedWarehouse }) => {
  ...
  const getWarehousePositions = () => {
    ...
    const rows = Math.ceil(warehouses.length / cols);
  ...
  const mapHeight =
    windowWidth <= 768
      ? Math.max(600, rows * 220 + 150)
      : Math.max(1000, rows * 170 + 150);
    ...
    >
      <style jsx>{`
        ...
        .warehouse-grid {
          display: grid;
          gap: 20px;
          padding: 10px; /* prevent edge cutoff */
        }
      ...
      {/* <div className="absolute bottom-4 left-4 bg-black bg-opacity-50 text-white px-3 py-2 rounded-lg z-10">
