...
import VerificationRackGrid from '@/components/storesManagement/storesVerification/VerificationRackGrid';
...
const Index = () => {
  ...
  const itemsRef = useRef(null); // ✅ NEW: Reference to items section
  ...
  // ✅ UPDATED: Auto-scroll to items section when rack is selected
  const handleRackSelect = (rack) => {
    console.log('Index: Rack selected:', rack);
    setSelectedRack(rack);

    // ✅ Scroll to items section after rack selection
    setTimeout(() => {
      if (itemsRef.current) {
    ...
    }, 300); // Small delay to ensure items section is rendered
  };
  ...
  // ✅ 1. Generate warehouse item list (WITHOUT balance) - matches StoreVerification1()
  const generateWarehouseItemList = async () => {
    ...
    try {
      ...
      const warehouseData = await verificationAPI.getWarehouseDetailsByWID(warehouseId);
      console.log('Warehouse data received:', warehouseData);

      console.log('Fetching warehouse items for ID:', warehouseId);
      const items = await verificationAPI.getWarehouseItemListByWID(warehouseId);
      ...
      if (!items || items.length === 0) {
        await Swal.fire({
          icon: 'info',
          title: 'No Items',
          text: 'No items found in this warehouse.',
          confirmButtonColor: '#fbbf24'
        });
      ...
      const worksheet = workbook.addWorksheet('Verification Sheet');
      ...
      // Header
      worksheet.getCell(`B${currentRow}`).value = 'WAREHOUSE ITEM LIST';
      ...
      worksheet.getCell(`C${currentRow}`).value = 'Code No.';
      worksheet.getCell(`D${currentRow}`).value = 'Description';
      worksheet.getCell(`E${currentRow}`).value = 'UOM';
      worksheet.getCell(`F${currentRow}`).value = 'Unit Price';
      ...
      items.forEach((item, index) => {
        try {
          ...
          const qty = parseFloat(getItemProperty(item, 'available_QTY', 'AVAILABLE_QTY', 'availableQty') || 0);
          const total = parseFloat(getItemProperty(item, 'total', 'Total', 'TOTAL') || (unitPrice * qty));

          worksheet.getCell(`B${currentRow}`).value = index + 1;
          worksheet.getCell(`C${currentRow}`).value = itemNumber;
          worksheet.getCell(`D${currentRow}`).value = description;
          worksheet.getCell(`E${currentRow}`).value = uom;
          worksheet.getCell(`F${currentRow}`).value = unitPrice.toFixed(2);
          worksheet.getCell(`G${currentRow}`).value = qty.toFixed(2);
          worksheet.getCell(`H${currentRow}`).value = total.toFixed(2);
      ...
      const warehouseCode = getItemProperty(warehouseData, 'w_CODE', 'W_CODE') ||
        getItemProperty(selectedWarehouse, 'w_CODE', 'W_CODE') ||
        'Warehouse';

      const filename = `VerificationSheet_${warehouseCode}_${Date.now()}.xlsx`;
      ...
      await Swal.fire({
        ...
        text: 'Warehouse item list has been downloaded successfully!',
        confirmButtonColor: '#10b981',
        timer: 2000,
        timerProgressBar: true
      });
    } catch (error) {
      console.error('Error generating warehouse item list:', error);
      await Swal.fire({
        icon: 'error',
        title: 'Download Failed',
        text: error.message || 'Failed to generate item list. Please try again.',
  ...
  // ✅ 2. Generate full verification sheet (WITH balance) - matches Storeverification()
  const generateFullVerificationSheet = async () => {
    if (!selectedRack) {
      await Swal.fire({
        icon: 'warning',
    ...
    <>
      <PageHeader>
        <div className="store-header-wrapper">
          <h1 className="page-title">Store Verification System</h1>
            ...
            <div ref={racksRef} className="section">
              ...
              <div className="download-section">
                <button
                  ...
                  onClick={generateWarehouseItemList}
                  disabled={downloading}
                >
                  {downloading && <div className="spinner"></div>}
                  📄 Download Warehouse Item List
                </button>

                {selectedRack && (
                  <button
                    className="download-btn secondary"
                    onClick={generateFullVerificationSheet}
                    disabled={downloading}
                  >
                    {downloading && <div className="spinner"></div>}
                    📊 Download Verification Sheet (Full)
                  </button>
                )}
              </div>

              <VerificationRackGrid
                warehouseId={selectedWarehouse.w_ID || selectedWarehouse.W_ID}
                onRackSelect={handleRackSelect}
                selectedRack={selectedRack}
              />

              {/* ✅ NEW: Items section with ref for auto-scroll */}
              {selectedRack && (
                <div ref={itemsRef} style={{ marginTop: '20px' }}>
                  {/* Items will be rendered inside VerificationRackGrid */}
                </div>
              )}
            </div>
          )}
